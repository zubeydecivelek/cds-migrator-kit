...
from cds_migrator_kit.videos.weblecture_migration.transform.xml_processing.rules.base import creators
from dojson.errors import IgnoreKey
from dojson.utils import force_list

# ATTENTION when COPYING! important which model you use as decorator
from ...models.video_lecture import model
from cds_migrator_kit.rdm.migration.transform.xml_processing.errors import MissingRequiredField, UnexpectedValue
...
from ..quality.contributors import (
    get_contributor_affiliation,
    get_contributor_role,
)
from cds_migrator_kit.rdm.migration.transform.xml_processing.quality.parsers import StringValue
...
def performer(self, key, value):
    ...
    affiliation = get_contributor_affiliation(value)
    name = value.get("a").strip()
    contributor = {
        "name": name,
        "role": "Performer" # TODO or "Participant"
    }
    if affiliation:
        contributor.update({"affiliation": affiliation})
    return contributor
...
def event_speakers(self, key, value):
    ...
    affiliations = get_contributor_affiliation(value)
    contributor = {
        "name": name,
        "role": "Speaker"
    }
    if affiliations:
        contributor.update({"affiliation": affiliations})
    return contributor
